from sympy import symbols, I, pi, sqrt, exp, cos, sin, Matrix, latex, simplify, expand
...
vector_ket_full_zeroes = Matrix([1] + [0] * (dim - 1))
vector_ket_full_ones = Matrix([0]*(dim-1) + [1])


normalized_ket_full_zeroes = (1/sqrt(2)) * vector_ket_full_zeroes
normalized_ket_full_ones = (1/sqrt(2)) * vector_ket_full_ones
product = Vk_tensor * (1/sqrt(2)) * (vector_ket_full_zeroes + vector_ket_full_ones)

test = normalized_dagger *normalized_Vkmatrix
simplified = simplify(expand(test, complex=True))
latex_rows = []
for i, val in enumerate(simplified):
    val_str = latex(val)
    latex_rows.append(f"{i+1} & {val_str} \\\\")
...
latex_result = latex(simplified, mat_str="pmatrix")

latex_expression = (
     f"{latex_result}"
)

html_path = "../matrices/tensorvk/application/tensrovk_HTML/Vk_tensor_applied_vector.html"
